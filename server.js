...
const square = (n) => {
    if (n < 0) 
        throw new Error("Cannot do square root of a negative number");
    return Math.sqrt(n);
};
...
app.get("/exponentiation", (req, res) => {
    try {
        ...
        logger.info(`Exponentiation: ${n1} ^ ${n2} = ${result}`);
        res.json({status: 200, result});
    }
    catch(error){
        sendErrorMessage(res, 400, error.message);
...
app.get("/square", (req, res) => {
    try {
        ...
        if (isNaN(n)) throw new Error("can not root of negative number");
        const result = square(n);

        logger.info(`Square: square(${n}) = ${result}`);
        res.json({status: 200, result});
    }
    catch(error){
        sendErrorMessage(res, 400, error.message);
